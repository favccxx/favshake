...
<head>
	...
	<title>现金红包-哎咦网赞助</title>
	<script type="text/javascript" charset="UTF-8" src="assets/js/prefixfree.min.js"></script>
<style type="text/css">
body {
...
.login-form:before {
    ...
    background:#fff url("assets/imgs/alipay.jpg") no-repeat 2px 2px;
}
.not-registered {
    position:absolute;
...
@font-face {
  font-family: 'loginform-icon';
  src: url("assets/font/loginform-icon.eot");
  src: url("assets/font/loginform-icon.eot?#iefix") format('embedded-opentype'),
       url("assets/font/loginform-icon.woff") format('woff'),
       url("assets/font/loginform-icon.ttf") format('truetype'),
       url("assets/font/loginform-icon.svg#loginform-icon") format('svg');
  font-weight: normal;
  font-style: normal;
}
</style>
...
<body>
	   ...
	   <div class="content">
           <form action="" method="post" class="login-form">
               <div class="username">
                   <input type="text" id="alipayAccount" name="alipayAccount" placeholder="支付宝账户" autocomplete="on" />
                   <span class="user-icon icon">u</span>
               </div>
               <div class="password">
                   <input type="text" id="cashValue" name="cashValue" value="" disabled="true" />
                   <span class="password-icon icon">p</span>
               </div>
               <div class="account-control">
                   <button type="button" onclick="getEnvelopCash()">领取</button>
               </div>
               <p class="not-registered">仅以支付宝形式发放，每日一次，第一次为准。<a>哎咦网赞助</a></p>
           </form>
	   </div>
...
<script type="text/javascript">
	window.onload = function() {
		var decryptData = '${redEnvelopValue }';
		var bytes  = CryptoJS.AES.decrypt(decryptData, 'ishake');
		var cashValueData = bytes.toString(CryptoJS.enc.Utf8);
		var myCash = cashValueData.substring(0, cashValueData.length-5) + " 元";
		$("#cashValue").attr("value",myCash);
	}

	function getEnvelopCash(){
		var alipayAccount = $("#alipayAccount").val();
		var cashValue = $("#cashValue").val();
		
		if(alipayAccount==""){
			alert("请填写您的支付宝账户，用于发放红包. \n\n谨记，系统在任何时候都不会以任何形式索要您的支付密码。");
			return;
		}
		...
		$.ajax({
			...
			if(result=="2"){
				alert("领取成功，系统将在3个工作日内发放到您的支付宝账户!");				
			}else if(result=="1"){
				alert("现金红包没人每日仅限一次。 \n\n想要更多现金红包，明天再来领取吧!");			
			}else if(result=="0"){
				alert("亲，请不要捣乱吆!");
